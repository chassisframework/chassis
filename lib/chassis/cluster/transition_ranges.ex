...
defmodule Chassis.Cluster.TransitionRanges do
  ...
  def release_range(
    %__MODULE__{active: active,
                released: released} = ranges,
    %RangeRelease{range: %Range{left: left, right: right}} = range_release
  ) do
    active = IntervalMap.delete(active, {left, right})
    released = IntervalMap.put(released, {left, right}, range_release)

    %__MODULE__{ranges | active: active, released: released, releasing: range_release}
  end
  ...
  def accept_range(
    ...
    active = IntervalMap.put(active, {left, right})

    %__MODULE__{ranges | active: active, stealing: nil}
  end
