defmodule Chassis.Cluster.Membership.Machine do
  ...
  def state_enter(
    _role,
    %ClusterState{
      state: {:transitioning, %Transition{current: {{:add_node, %Node{name: node}}, confirmation_message}}}
    } = state
  ) when node == node() do
    # since effects are only executed by the master, we have to explicitly perform these actions
  ...
  def apply(_meta, {:stage, :commit}, %ClusterState{state: :running, staged_changes: staged_changes} = state) when length(staged_changes) > 0 do
    effects = [inform_subscribers_effect(state, {:stage_commit, staged_changes}),
               compute_new_layout_effect(state)]
  ...
  def apply(_meta, :destroy_cluster, state) do
    effects = [destroy_partitions_effect(state),
               destroy_membership_group_effect(state),
               inform_subscribers_effect(state, :destroying_cluster)]
  ...
  defp execute_transition_step_effects(
    {{:add_partition,
      %NewPartition{
        id: partition_id,
        ranges: ranges,
